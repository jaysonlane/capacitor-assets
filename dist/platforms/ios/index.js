...
exports.IOS_APP_ICON_SET_PATH = `Images.xcassets/${exports.IOS_APP_ICON_SET_NAME}.appiconset`;
exports.IOS_SPLASH_IMAGE_SET_NAME = 'Splash';
exports.IOS_SPLASH_IMAGE_SET_PATH = `Images.xcassets/${exports.IOS_SPLASH_IMAGE_SET_NAME}.imageset`;
class IosAssetGenerator extends asset_generator_1.AssetGenerator {
    ...
    async generate(asset, project) {
        ...
        switch (asset.kind) {
            ...
            case "icon" /* AssetKind.Icon */:
                return [];
            case "settings-icon" /* AssetKind.SettingsIcon */:
                return this.generateSettingsIcons(asset, project);
            case "spotlight-icon" /* AssetKind.SpotlightIcon */:
    ...
    async generateIconsForLogo(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => ["icon" /* AssetKind.Icon */, "notification-icon" /* AssetKind.NotificationIcon */, "settings-icon" /* AssetKind.SettingsIcon */, "spotlight-icon" /* AssetKind.SpotlightIcon */].find((i) => i === a.kind));
    ...
    async generateNotificationIcons(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => a.kind === "notification-icon" /* AssetKind.NotificationIcon */);
        return this._generateIcons(asset, project, icons);
    }
    async generateSettingsIcons(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => a.kind === "settings-icon" /* AssetKind.SettingsIcon */);
        return this._generateIcons(asset, project, icons);
    }
    async generateSpotlightIcons(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => a.kind === "spotlight-icon" /* AssetKind.SpotlightIcon */);
        return this._generateIcons(asset, project, icons);
    }
    ...
    async updateIconsContentsJson(generated, project) {
        var _a;
        const contentsJsonPath = (0, path_1.join)(project.config.ios.path, exports.IOS_APP_ICON_SET_PATH, 'Contents.json');
        ...
        for (const g of generated) {
            const existing = withoutMissing.find((f) => {
                var _a, _b;
                return f.scale === `${g.template.scale}x` &&
                    f.size === `${g.template.width / ((_a = g.template.scale) !== null && _a !== void 0 ? _a : 1)}x${g.template.height / ((_b = g.template.scale) !== null && _b !== void 0 ? _b : 1)}` &&
                    f.idiom === g.template.idiom &&
